...
def getopts() :
  ...
  # if a password starts with 'asm:' shell out and decrypt it using decrypt_secretid.rb
  if (re.match('^file:', user) or re.match('^(file|asm):', password)):
        if re.match('^file:', user):
          ...
          password = filetext[0]
          filename.close()
        elif re.match('^asm:', password):
          cryptextract = re.sub('^asm:', '', password)
          proc = subprocess.Popen(['/opt/asm-deployer/scripts/decrypt_secretid.rb', cryptextract], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
          password = proc.communicate()[0].rstrip()

          if (proc.wait() != 0):
            print 'Unknown: Could not decrypt the ASM password'
...
if os_platform != "win32":
  ...
  def handler(signum, frame):
    print 'Error: Execution time too long!'
      ...
      if vendor == "hp" :
        if instance['HealthState'] is not None :
          ...
          if (interpretStatus == ExitCritical) :
            ...
            GlobalStatus = ExitCritical
            ExitMsg += " Error: %s " % elementNameValue
          if (interpretStatus == ExitWarning and GlobalStatus != ExitCritical) :
            ...
            ExitMsg += " Degraded: %s " % elementNameValue
          ...
          if (interpretStatus == ExitWarning and GlobalStatus == ExitCritical) : # ARR
            ExitMsg += " Degraded: %s " % elementNameValue #ARR
