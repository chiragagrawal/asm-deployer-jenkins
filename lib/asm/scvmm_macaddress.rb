...
result = winrm.powershell("$Host.UI.RawUI.BufferSize = New-Object Management.Automation.Host.Size (500, 25); Import-Module VirtualMachineManager; Get-VMMServer -ComputerName localhost; Refresh-VM -VM #{@opts[:vmname]}; Get-VM -Name #{@opts[:vmname]} | Select -ExpandProperty Virtualnetworkadapters | Where-Object { $_.slotid -eq 0 } | Select MACAddress")
