...
module ASM
  module Cipher
    ...
    def self.load_secrets
      if defined?(ASM.secrets)
        ASM.secrets
      else
        # handle the case where only asm/cipher has been included
        config = ASM::Config.new
        ASM::Secrets.create(config)
      end
    end
    ...
    def self.decrypt_string(id)
      secrets.decrypt_string(id)
    end

    def self.decrypt_credential(id)
      secrets.decrypt_credential(id)
    end

    def self.create_token(string)
      {
          'token' => "ASMTOKEN-#{ASM::PrivateUtil.uuid}",
  ...
  module CipherLocal
    ...
    def self.get_encrypted_string(id)
      ...
      raise(ASM::NotFoundException, "Invalid encryption string id: '#{id}'") unless result.is_a? ::Hash
      result
    end
    ...
    def self.get_encryption_key(id)
      ...
      raise(ASM::NotFoundException, "Invalid encryption key id: '#{id}'") unless result.is_a? ::Hash
      result
    end
...
EOT
    ...
    def self.decrypt_credential(id)
      ...
      raise(ASM::NotFoundException, "Invalid credential id: '#{id}'") unless result.is_a? ::Hash
      ret = {}
      result.each do |key, val|
        if !val.nil? && key.to_s =~ /^(.+?)_?id$/
