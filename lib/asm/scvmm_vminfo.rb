...
result = winrm.powershell("$Host.UI.RawUI.BufferSize = New-Object Management.Automation.Host.Size (500, 25); Import-Module VirtualMachineManager; $vmm = Get-VMMServer -ComputerName localhost; $refresh = Refresh-VM -VM #{@opts[:vmname]}; $vm = Get-VM -Name #{@opts[:vmname]} | Select OperatingSystem | ConvertTo-JSON; Write-Host $vm")
